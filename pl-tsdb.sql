...
--------------------------------------------------------
   ...
   CREATE SEQUENCE  "TSDB_METRIC_SEQ"  MINVALUE 1 MAXVALUE 9999999999999999999999999999 INCREMENT BY 1 START WITH 101 CACHE 20 NOORDER  NOCYCLE ;
--------------------------------------------------------
...
-- Unable to render TABLE DDL for object TQREACTOR.TSDB_METRIC_TAGSTORE with DBMS_METADATA attempting internal generator.
CREATE INDEX SYS_FK0000116396N00004$ ON TSDB_METRIC_TAGSTORE (NESTED_TABLE_ID ASC) 
LOGGING 
TABLESPACE USERS 
PCTFREE 10 
INITRANS 2 
...
NOPARALLEL
--------------------------------------------------------
--  DDL for Index SYS_C0033865
--------------------------------------------------------

  CREATE UNIQUE INDEX "SYS_C0033865" ON "TSDB_CONFIG" ("K") 
...
--------------------------------------------------------
  ...
  TABLESPACE "USERS" ;
--------------------------------------------------------
--  DDL for Index SYS_C0033871
--------------------------------------------------------

  CREATE UNIQUE INDEX "SYS_C0033871" ON "TSDB_METRIC_QUEUE" ("SYS_NC0000400005$") 
  PCTFREE 10 INITRANS 2 MAXTRANS 255 
  STORAGE(INITIAL 65536 NEXT 1048576 MINEXTENTS 1 MAXEXTENTS 2147483645
  PCTINCREASE 0 FREELISTS 1 FREELIST GROUPS 1 BUFFER_POOL DEFAULT FLASH_CACHE DEFAULT CELL_FLASH_CACHE DEFAULT)
  TABLESPACE "USERS" ;
--------------------------------------------------------
--  DDL for Index SYS_FK0000116396N00004$
--------------------------------------------------------

  CREATE INDEX "SYS_FK0000116396N00004$" ON "TSDB_METRIC_TAGSTORE" ("NESTED_TABLE_ID") 
...
--------------------------------------------------------
  ...
  ALTER TABLE "TSDB_CONFIG" ADD PRIMARY KEY ("K")
...
--------------------------------------------------------
  ...
  ALTER TABLE "TSDB_METRIC_QUEUE" ADD UNIQUE ("M"."TAGS")
  USING INDEX PCTFREE 10 INITRANS 2 MAXTRANS 255 
  STORAGE(INITIAL 65536 NEXT 1048576 MINEXTENTS 1 MAXEXTENTS 2147483645
  PCTINCREASE 0 FREELISTS 1 FREELIST GROUPS 1 BUFFER_POOL DEFAULT FLASH_CACHE DEFAULT CELL_FLASH_CACHE DEFAULT)
  TABLESPACE "USERS"  ENABLE;
  ALTER TABLE "TSDB_METRIC_QUEUE" ADD PRIMARY KEY ("METRIC_ID")
  USING INDEX PCTFREE 10 INITRANS 2 MAXTRANS 255 
  STORAGE(INITIAL 65536 NEXT 1048576 MINEXTENTS 1 MAXEXTENTS 2147483645
  PCTINCREASE 0 FREELISTS 1 FREELIST GROUPS 1 BUFFER_POOL DEFAULT FLASH_CACHE DEFAULT CELL_FLASH_CACHE DEFAULT)
  TABLESPACE "USERS"  ENABLE;
...
--  FUNCTION START_ELAPSED_METRIC(mdepth IN INT, metrics IN METRIC_ARR) RETURN METRIC;
  ...
  --====================================================================================================
  FUNCTION PIPE_METRICS(batchsize IN INT DEFAULT 1000) RETURN METRIC_EXT_ARR PIPELINED PARALLEL_ENABLE;
...
--------------------------------------------------------
  ...
  -- Pipes out a batch of persisted metrics
  --====================================================================================================
  FUNCTION PIPE_METRICS(batchsize IN INT DEFAULT 1000) RETURN METRIC_EXT_ARR PIPELINED PARALLEL_ENABLE IS
    met METRIC_EXT;
    done PLS_INTEGER := 0;
    maxrows CONSTANT PLS_INTEGER := batchsize;
    CURSOR metricsCursor IS SELECT METRIC_EXT(METRIC_ID, ROWIDTOCHAR(ROWID), M) FROM TSDB_METRIC_QUEUE FOR UPDATE SKIP LOCKED;
  BEGIN
    OPEN metricsCursor;
    LOOP
      FETCH metricsCursor INTO met;
      EXIT WHEN metricsCursor%NOTFOUND OR done = maxrows;
      PIPE ROW (met);
      done := done +1;
    END LOOP;
    CLOSE metricsCursor;
  ...
  BEGIN
    FORALL i IN 1..metrics.COUNT 
      INSERT INTO TSDB_METRIC_QUEUE (METRIC_ID, M) VALUES (TSDB_METRIC_SEQ.NEXTVAL , metrics(i));
    COMMIT;
  END SAVE_METRICS;
...
--          DBMS_SQL.DEFINE_COLUMN(cursorNum, colcnt, tsNumber, DBMS_TYPES.TYPECODE_NUMBER);
    ...
    EXCEPTION WHEN OTHERS THEN       
      DECLARE
        errm VARCHAR2(200) := SQLERRM();
      BEGIN
        LOGGING.tcplog('CURSORINTTOMETRICS ERROR: errm:' || errm || ', backtrace:' || dbms_utility.format_call_stack);
